		...
		IPropertyPage
{
	...
	private transient Button unionButton;
	...
	final private static String TEXT_UNION = "Union";
	...
	private void createRadioButtonList( Composite composite )
	{
		...
		data = new GridData( );
		data.grabExcessVerticalSpace = true;
		unionButton = new Button( composite, SWT.RADIO | SWT.WRAP );
		unionButton.setText( TEXT_UNION );
		unionButton.setLayoutData( data );
		...
		unionButton.addSelectionListener(listener);;
	}
	...
	/**
	 * get all data set list including those that should be referenced from
	 * librarys
	 * 
	...
	private List getDataSets( )
	{
		...
		while ( handleIter.hasNext( ) )
		{
			...
			if ( handleObj instanceof JointDataSetHandle )
			{
				...
				{
					DataSetHandle dataSet = Utility.findDataSet( nameList.get( j )
							.toString( ) );

					if ( dataSet != null
							&& dataSet.getModuleHandle( ) instanceof LibraryHandle )
	 ...
	 * @return
	 */
	public static String getParameterName( String dataSetName,
	...
	class RadioSelectionLister implements SelectionListener
	{
		...
		public void widgetSelected( SelectionEvent e )
		{
			...
			if ( e.getSource( ) instanceof Button )
			{
				...
				else if ( ( (Button) e.getSource( ) ).equals( unionButton ) )
					joinType = DesignChoiceConstants.JOIN_TYPE_UNION;
	...
	public void pageActivated( )
	{
		if ( this.propertyPageContainer != null )
		{
			...
			while ( conditionIter.hasNext( ) )
			{
				...
				populateDataSet( condition.getLeftDataSet( ),
						condition.getLeftExpression( ),
						LEFT_DATASET );
				populateDataSet( condition.getRightDataSet( ),
	...
	private void populateJoinType( String joinType )
	{
		...
		else if ( joinType.equals( DesignChoiceConstants.JOIN_TYPE_UNION ) )
		{
			innerJoinButton.setSelection( false );
			leftOuterJoinButton.setSelection( false );
			rightOuterJoinButton.setSelection( false );
	...
	private void populateDataSet( String name, String expression, int type )
	{
		...
		if ( type == LEFT_DATASET )
		{
			if ( selectionIndex >= 0 )
			{
				...
				if ( leftColumnList.getInput( ) != null
		...
		if ( type == RIGHT_DATASET )
		{
			if ( selectionIndex >= 0 )
			{
				...
				if ( rightColumnList.getInput( ) != null
