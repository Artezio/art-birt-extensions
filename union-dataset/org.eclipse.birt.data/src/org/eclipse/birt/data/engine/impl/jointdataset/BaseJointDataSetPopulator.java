...
public class BaseJointDataSetPopulator implements IDataSetPopulator
{
	...
	private IResultObject curSecondaryResultObject = null;
	...
	private JointResultMetadata getMeta(JointResultMetadata meta) {
		if (joinType == IJointDataSetDesign.UNION) {
		}
		return meta;
	}
	...
	private IResultObject nextForUnion() throws DataException {
		IResultObject result = null;
		curPrimaryResultObject = primaryIterator.getCurrentResult();
		if (curPrimaryResultObject != null) {
			primaryIterator.next();
			result = createResultObject(curPrimaryResultObject, null);
		} else {
			curSecondaryResultObject = secondaryIterator.getCurrentResult();
		}
		
		if (curSecondaryResultObject != null) {
			secondaryIterator.next();
			result = createResultObject(curSecondaryResultObject, null);
		}
		return result;
	}
			...
			IResultObject secondary ) throws DataException
	{
		...
		for ( int i = 1; i <= fields.length; i++ )
		{
			...
			int sourceIndex = meta.getSourceIndex( i );
			if (ri != null && ri.getResultClass().getFieldCount() >= sourceIndex) {
				fields[i - 1] = ri.getFieldValue( sourceIndex);
			}
